...
// Update area in plant
router.patch('/:companyId/:id/areas/:areaId', authenticate, checkCompanyAccess, authorize(['company_owner', 'plant_head']), async (req, res) => {
  try {
    ...
    const area = plant.areas.id(areaId);
    if (!area) {
...
// Delete area from plant
router.delete('/:companyId/:id/areas/:areaId', authenticate, checkCompanyAccess, authorize(['company_owner', 'plant_head']), async (req, res) => {
  try {
    const { companyId, id, areaId } = req.params;
...
router.get('/:companyId/:id/areas', authenticate, checkCompanyAccess, async (req, res) => {
  try {
    const { companyId, id } = req.params;
